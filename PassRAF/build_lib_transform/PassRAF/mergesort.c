...
#pragma css task in(low1[tmp_size1], tmp1, low2[tmp_size2], split2)\
								out(lowdest[tmp_size1+tmp_size2]) \
								safe(tmp1, split2)
...
#pragma css task in(tmp1[tmp_size1], high1, tmp2[tmp_size2], high2)\
								out(tmp3[tmp_size1+tmp_size2]) \
								safe(high1, high2)
...
#pragma css task inout(A[quarter]) out(tmpA[quarter]) \
								 in(quarter)
	sort(A, tmpA, quarter);
#pragma css task inout(B[quarter]) out(tmpB[quarter]) \
 								 in(quarter)
	sort(B, tmpB, quarter);
#pragma css task inout(C[quarter]) out(tmpC[quarter]) \
 								 in(quarter)
	sort(C, tmpC, quarter);
	long tmp_size = size - 3 * quarter;
#pragma css task inout(D[tmp_size]) out(tmpD[tmp_size]) \
 								 in(tmp_size)
...
/*#pragma css sync*/
	...
	tmp1 = A + quarter - 1;
	tmp2 = B + quarter - 1;
#pragma css task in(A[quarter], tmp1, B[quarter], tmp2) \
								 out(tmpA[2*quarter]) safe(tmp1, tmp2)                     \
								 safe(tmp1, tmp2)
	merge(A, tmp1, B, tmp2, tmpA);

	tmp1 = C + quarter - 1;
	tmp2 = low + size - 1;
#pragma css task in(C[quarter], tmp1,	D[tmp_size], tmp2) \
								 out(tmpC[quarter+tmp_size]) safe(tmp1, tmp2) \
								 safe(tmp1, tmp2)
...
#pragma css task in(tmpA[2*quarter], tmp1, tmpC[tmp_size+quarter], tmp2) \
										out(A[size]) \
								 safe(tmp1, tmp2)
...
#pragma css sync
	;
#pragma css task inout(array[size]) in(size)
	fill_array(array, size);
	printf("Initialization Done!");

	/* Timing. "Start" timers */
	START_TIME();
#pragma css task inout(array[size]) out(tmp[size]) in(size)
	sort(array, tmp, size);
